...
from utils.save_response import save_response_to_file
# Setup logger
...
def analyze_stocks(agent, stocks, agent_name="Agent"):
    ...
    try:
        ...
        filename = save_response_to_file(response, agent_name, stocks)
        
        # Log the response summary
        logger.info(f"{agent_name} analysis complete")
        if filename:
            logger.info(f"{agent_name} response saved to {filename}")
